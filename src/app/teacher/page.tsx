...
import {
    ...
    ChevronRight, Search, Settings, Bell, Atom,
    FileText, PenTool, Check, X, Shield, Filter, ArrowLeft, ArrowRight,
    Briefcase, CalendarPlus, Loader2, Megaphone, AlertTriangle,
    User, Mail, Phone, MapPin, Award, BookOpen, Star, ChevronDown, Edit2, Save, Layers,
    LogOut, Globe, Lock, Moon, Sun, Laptop, Volume2, Trash2, Zap, Layout, Eye
} from "lucide-react";
import { MOCK_CLASS_LIST, PENDING_LEAVES, TEACHER_DIRECTORY, TEACHER_SCHEDULE } from "@/lib/data";
...
// --- TYPES ---
type ActivityItem = { id: number; time: string; action: string; detail: string; icon: any; color: string };
...
const TopNavigation = ({ onViewChange }: { onViewChange: (view: string) => void }) => {
    ...
    const [notifications, setNotifications] = useState([
        ...
        { id: 2, title: "System Update", desc: "Gradebook module updated to v2.4.", time: "2h ago", icon: Layers, color: "text-blue-500 bg-blue-500/10" },
        ...
        <header className="fixed top-0 left-0 right-0 h-24 flex items-center justify-between px-6 z-40 pointer-events-none">
            ...
            <div className="relative z-10 flex items-center gap-4 pointer-events-auto">
                ...
                <div>
                    ...
                    <span className="text-[9px] font-mono text-indigo-400 uppercase tracking-widest">Faculty Access</span>
                </div>
            </div>

            <div className="relative z-10 flex items-center gap-3 pointer-events-auto" onClick={(e) => e.stopPropagation()}>
                <button onClick={() => onViewChange('settings')} className="p-3 rounded-full bg-white/[0.03] border border-white/[0.05] text-neutral-400 hover:text-white hover:bg-white/10 transition-colors">
                    <Settings size={18} />
                </button>
                ...
                <div className="relative">
                    <button onClick={() => { setIsProfileOpen(!isProfileOpen); setIsNotifOpen(false); }} className={`w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 border border-white/10 ml-2 transition-transform active:scale-95 ${isProfileOpen ? 'ring-2 ring-white/20' : ''}`} />
                    <AnimatePresence>
                        {isProfileOpen && (
...
// --- COMPONENT: SETTINGS MODAL (UPGRADED) ---
const SettingsModal = ({ onClose }: { onClose: () => void }) => {
    useScrollLock();
    // FIXED: Removed 'addToast' from useConfig()
    const { setThemeColor } = useConfig();
    const { addToast: toast } = useToast();

    const [activeTheme, setActiveTheme] = useState('dark');
    const [activeColor, setActiveColor] = useState('indigo');
    const [toggles, setToggles] = useState({ push: true, email: false, sound: true, reducedMotion: false });
    ...
    const handleColorChange = (id: string) => {
        setActiveColor(id);
        toast(`System Accent Updated: ${id.charAt(0).toUpperCase() + id.slice(1)}`, "success");
    };
    ...
    return (
        <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="fixed inset-0 z-[60] bg-[#050505] flex flex-col text-white">
            ...
            <div className="p-6 border-b border-white/[0.05] flex justify-between items-center bg-[#050505]/90 backdrop-blur-xl sticky top-0 z-50">
                ...
                <span className="font-mono text-xs uppercase tracking-[0.2em] text-neutral-500">System Preferences</span>
                <div className="w-10" />
            </div>

            <div className="flex-1 overflow-y-auto p-8 max-w-2xl mx-auto w-full pb-20 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/20">
                ...
                <div className="mb-10">
                    <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                        <Eye size={14} /> Interface Theme
                    </h3>
                    <div className="grid grid-cols-3 gap-4">
                        {[
                            { id: 'dark', icon: Moon, label: 'Dark Mode' },
                            { id: 'light', icon: Sun, label: 'Light Mode' },
                            { id: 'system', icon: Laptop, label: 'Auto Sync' }
                        ].map((t) => (
                            <button
                                key={t.id}
                                onClick={() => setActiveTheme(t.id)}
                                className={`relative p-6 rounded-2xl border transition-all duration-300 flex flex-col items-center gap-3 group ${activeTheme === t.id ? 'bg-white/[0.08] border-white/20 shadow-xl shadow-black/50' : 'bg-white/[0.02] border-white/5 hover:bg-white/[0.04]'}`}
                            >
                                <div className={`p-3 rounded-full transition-colors ${activeTheme === t.id ? 'bg-white text-black' : 'bg-white/5 text-neutral-400 group-hover:text-white'}`}>
                                    <t.icon size={20} />
                                </div>
                                <span className={`text-xs font-bold ${activeTheme === t.id ? 'text-white' : 'text-neutral-500 group-hover:text-white'}`}>{t.label}</span>
                                {activeTheme === t.id && <div className="absolute top-3 right-3 w-2 h-2 bg-emerald-500 rounded-full shadow-[0_0_8px_#10B981]" />}
                            </button>
                        ))}
                    </div>
                </div>

                {/* Section 2: Command Color */}
                <div className="mb-10">
                    <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                        <Zap size={14} /> Command Color
                    </h3>
                    <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
                        {colors.map((c) => (
                            <button
                                ...
                                className={`group relative w-16 h-16 rounded-2xl flex items-center justify-center border transition-all duration-300 ${activeColor === c.id ? 'bg-white/[0.05] border-white/20 scale-105' : 'border-transparent hover:bg-white/[0.02]'}`}
                            >
                                <div className={`w-6 h-6 rounded-full ${c.hex} shadow-[0_0_15px_currentColor] transition-transform group-hover:scale-110`} />
                                {activeColor === c.id && <div className="absolute -bottom-2 w-1 h-1 bg-white rounded-full" />}
                            </button>
                        ))}
                    </div>
                </div>
                ...
                <div className="mb-10">
                    <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                        <Bell size={14} /> Notification Matrix
                    </h3>
                    <div className="space-y-3">
                        {[
                            { id: 'push', l: "Push Notifications", d: "Real-time alerts on your device" },
                            { id: 'email', l: "Email Digests", d: "Daily summary of classroom activity" },
                            { id: 'sound', l: "Haptic & Sound", d: "Feedback for interactions" }
                        ].map((opt) => (
                            <div key={opt.id} onClick={() => handleToggle(opt.id as any)} className="flex items-center justify-between p-5 bg-white/[0.02] border border-white/5 rounded-2xl cursor-pointer hover:bg-white/[0.04] transition-colors group">
                                <div>
                                    <p className="text-sm font-bold text-white group-hover:text-indigo-400 transition-colors">{opt.l}</p>
                                    <p className="text-[10px] text-neutral-500">{opt.d}</p>
                                </div>
                                {/* Custom Toggle Switch */}
                                <div className={`w-12 h-7 rounded-full p-1 transition-colors duration-300 flex items-center ${toggles[opt.id as keyof typeof toggles] ? 'bg-indigo-600' : 'bg-white/10'}`}>
                                    <motion.div
                                        layout
                                        className="w-5 h-5 bg-white rounded-full shadow-md"
                                        animate={{ x: toggles[opt.id as keyof typeof toggles] ? 20 : 0 }}
                ...
                <div>
                    <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                        <Shield size={14} /> Security Core
                    </h3>
                    <div className="p-1 bg-white/[0.02] border border-white/5 rounded-2xl">
                        <button onClick={() => toast("Reset Link Sent", "success")} className="w-full flex items-center justify-between p-4 hover:bg-white/5 rounded-xl transition-colors text-left group">
                            <div className="flex items-center gap-4">
                                <div className="p-2 bg-indigo-500/10 rounded-lg text-indigo-400"><Lock size={18} /></div>
                                <div>
                                    <p className="text-sm font-bold text-white">Change Password</p>
                                    <p className="text-[10px] text-neutral-500">Last updated: 30 days ago</p>
                                </div>
                            </div>
                            <ChevronRight size={16} className="text-neutral-600 group-hover:text-white" />
                        </button>
                        <div className="h-px bg-white/5 mx-4" />
                        <button onClick={() => toast("Cache Cleared", "info")} className="w-full flex items-center justify-between p-4 hover:bg-white/5 rounded-xl transition-colors text-left group">
                            <div className="flex items-center gap-4">
                                <div className="p-2 bg-rose-500/10 rounded-lg text-rose-500"><Trash2 size={18} /></div>
                                <div>
                                    <p className="text-sm font-bold text-white">Clear Local Cache</p>
                                    <p className="text-[10px] text-neutral-500">Free up device storage</p>
                                </div>
                            </div>
                            <ChevronRight size={16} className="text-neutral-600 group-hover:text-white" />
                        </button>
                ...
                <div className="pt-12 text-center">
                    <p className="text-[10px] text-neutral-600 font-mono uppercase tracking-widest">NexGen OS v16.1.1 • Secure Build</p>
                </div>
            </div>
        </motion.div>
    );
...
// --- COMPONENT: TEACHER PROFILE (EDITABLE & PERSISTENT) ---
const TeacherProfile = ({ onClose }: { onClose: () => void }) => {
    ...
    return (
        <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="fixed inset-0 z-[60] bg-[#050505] flex flex-col text-white">
            ...
            <div className="flex-1 overflow-y-auto p-6 max-w-3xl mx-auto w-full pb-20 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/20">
                <div className="flex flex-col items-center text-center mb-12 mt-4">
                    ...
                    {isEditing ? (
                        <input
                            ...
                            className="text-4xl font-bold tracking-tight mt-6 mb-2 bg-transparent border-b border-white/20 text-center focus:border-indigo-500 outline-none w-full max-w-md transition-colors"
                        />
                    ) : (
                        <h1 className="text-4xl font-bold tracking-tight mt-6 mb-2">{profileData.name}</h1>
                    )}
                ...
                <div className="grid grid-cols-3 gap-4 mb-8">
                    <div className="p-5 rounded-[2rem] bg-white/[0.02] border border-white/[0.05] flex flex-col items-center justify-center hover:bg-white/[0.04] transition-colors">
                        <Briefcase className="text-blue-500 mb-3" size={24} />
                        <p className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">Experience</p>
                        <h4 className="text-2xl font-black mt-2 tracking-tighter">{profileData.experience}</h4>
                    </div>
                    <div className="p-5 rounded-[2rem] bg-white/[0.02] border border-white/[0.05] flex flex-col items-center justify-center hover:bg-white/[0.04] transition-colors">
                        <BookOpen className="text-emerald-500 mb-3" size={24} />
                        <p className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">Classes</p>
                        <h4 className="text-2xl font-black mt-2 text-emerald-400 tracking-tighter">{profileData.classes.length}</h4>
                    </div>
                    <div className="p-5 rounded-[2rem] bg-white/[0.02] border border-white/[0.05] flex flex-col items-center justify-center hover:bg-white/[0.04] transition-colors">
                        ...
                        <h4 className="text-2xl font-black mt-2 text-yellow-500 tracking-tighter">{profileData.rating}</h4>
                    </div>
                </div>

                <div className="bg-white/[0.02] border border-white/[0.05] rounded-[2.5rem] p-8 space-y-6 relative group">
                    <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
                        <h3 className="text-sm font-bold text-white uppercase tracking-widest">Personal Details</h3>
                        <button
                            onClick={() => isEditing ? handleSave() : setIsEditing(true)}
                            disabled={isLoading}
                            className={`flex items-center gap-2 px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${isEditing ? 'bg-emerald-500 text-black hover:bg-emerald-400' : 'bg-white/5 text-neutral-400 hover:text-white hover:bg-white/10'}`}
                        >
                        ...
                        <div className="w-full">
                            ...
                            {isEditing ? (
                                <input
                                    ...
                                    className="w-full bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-indigo-500 outline-none transition-colors"
                                />
                            ) : (
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="w-full">
                            ...
                            {isEditing ? (
                                ...
                                <p className="text-sm text-white font-medium">{profileData.office}</p>
                            )}
                        </div>
                    </div>

                    <div className="flex items-center gap-4">
                        <div className="p-3 bg-white/5 rounded-xl text-neutral-400 shrink-0"><Award size={20} /></div>
                        <div className="w-full">
                            <p className="text-[10px] text-neutral-500 uppercase font-bold mb-1">Qualifications</p>
                            {isEditing ? (
                                <input
                                ...
                                <p className="text-sm text-white font-medium">{profileData.qualification}</p>
                            )}
                        </div>
                    </div>
                </div>
...
const AttendanceRegister = ({ onClose, classId, onLog }: { onClose: () => void, classId: string, onLog: (a: string, d: string) => void }) => {
    ...
    const [students, setStudents] = useState(
        MOCK_CLASS_LIST.filter(student => student.class === classId)
    );
    ...
    return (
        <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="fixed inset-0 z-[60] bg-[#050505] flex flex-col text-white">
            ...
            <div className="flex-1 overflow-y-auto p-6 max-w-3xl mx-auto w-full pb-24 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/20">
                <div className="space-y-3">
                        ...
                        <div key={student.id} onClick={() => toggleStatus(student.id)} className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.02] border border-white/[0.05] cursor-pointer hover:bg-white/[0.05] transition-colors active:scale-[0.99]">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-neutral-600 font-mono uppercase">{student.id}</p>
                                </div>
                            </div>

                            <div className={`px-4 py-2 rounded-xl text-xs font-bold w-24 text-center transition-colors ${student.status === "Present" ? "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20" :
                                student.status === "Absent" ? "bg-red-500/10 text-red-500 border border-red-500/20" :
            ...
            <div className="p-6 border-t border-white/[0.05] bg-[#050505]/90 backdrop-blur-xl absolute bottom-0 w-full">
                <button onClick={submitRegister} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-900/20">
                    Sync Attendance
                </button>
            </div>
        </motion.div>
                        ...
                        <div key={student.id} className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.02] border border-white/[0.05]">
                            ...
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    className="w-16 bg-black border border-white/10 rounded-lg py-2 text-center font-mono text-sm focus:border-indigo-500 outline-none transition-colors"
                                />
...
const BroadcastComposer = ({ onClose, classId, onLog }: { onClose: () => void, classId: string, onLog: (a: string, d: string) => void }) => {
    ...
    return (
        <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="fixed inset-0 z-[60] bg-[#050505] flex flex-col text-white">
            ...
            <form onSubmit={handleSend} className="flex-1 p-8 max-w-xl mx-auto w-full mt-8 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/20">
                <div className="mb-8">
                ...
                <div className="space-y-6">
                    <div className="space-y-3">
                        ...
                        <div className="flex gap-4">
                            <button
                                ...
                                className={`flex-1 p-4 rounded-xl border flex items-center justify-center gap-2 transition-all ${!isUrgent ? 'bg-white/10 border-white/20 text-white' : 'bg-transparent border-white/5 text-neutral-500'}`}
                            >
                                <Bell size={16} /> Normal
                            </button>
                            <button
                                ...
                                className={`flex-1 p-4 rounded-xl border flex items-center justify-center gap-2 transition-all ${isUrgent ? 'bg-rose-500/20 border-rose-500 text-rose-500' : 'bg-transparent border-white/5 text-neutral-500'}`}
                            >
                    ...
                    <div className="space-y-3">
                        ...
                        <textarea
                            ...
                            rows={6}
                            className="w-full bg-white/5 border border-white/10 rounded-xl p-5 outline-none focus:border-indigo-500 transition-colors resize-none text-sm leading-relaxed"
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-indigo-600 text-white rounded-xl font-bold flex justify-center hover:bg-indigo-500 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed mt-4"
                    >
            ...
            <div className="flex-1 overflow-y-auto p-6 max-w-3xl mx-auto w-full pb-20 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/20">
                {requests.length === 0 ? (
                    <div className="text-center mt-20 opacity-50">
                        ...
                        <p className="text-sm">All student requests processed.</p>
                    </div>
                ) : (
                        ...
                        {requests.map((req) => (
                            <div key={req.id} className="p-6 bg-white/[0.03] border border-white/[0.05] rounded-[2rem]">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 className="text-lg font-bold text-white">{req.student}</h3>
                                        <p className="text-xs text-indigo-400 font-mono mt-1 uppercase tracking-wide">{req.type} Leave • {req.date}</p>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <button onClick={() => handleDecision(req.id, 'rejected')} className="py-3 rounded-xl border border-white/10 hover:bg-white/5 text-neutral-400 font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2">
                                        <X size={14} /> Reject
                                    </button>
                                    <button onClick={() => handleDecision(req.id, 'approved')} className="py-3 rounded-xl bg-indigo-600 text-white font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-indigo-500">
...
const TeacherLeaveRequestModal = ({ onClose }: { onClose: () => void }) => {
    ...
    return (
        <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="fixed inset-0 z-[60] bg-[#050505] flex flex-col text-white">
            ...
            <form onSubmit={handleSubmit} className="p-8 max-w-lg mx-auto w-full space-y-6 mt-10 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/20">
                <h2 className="text-3xl font-bold mb-8 tracking-tight">Request Time Off</h2>
                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2"><label className="text-[10px] uppercase font-bold text-neutral-500 tracking-wider">From</label><input required type="date" className="w-full bg-white/5 border border-white/10 rounded-xl p-4 outline-none focus:border-indigo-500 transition-colors [color-scheme:dark]" /></div>
                    <div className="space-y-2"><label className="text-[10px] uppercase font-bold text-neutral-500 tracking-wider">To</label><input required type="date" className="w-full bg-white/5 border border-white/10 rounded-xl p-4 outline-none focus:border-indigo-500 transition-colors [color-scheme:dark]" /></div>
                </div>
                <div className="space-y-2"><label className="text-[10px] uppercase font-bold text-neutral-500 tracking-wider">Type</label>
                    <select className="w-full bg-white/5 border border-white/10 rounded-xl p-4 outline-none focus:border-indigo-500 transition-colors text-neutral-300">
                        <option>Medical Leave</option>
                        <option>Casual Leave</option>
                        <option>Professional Development</option>
                    </select>
                </div>
                <div className="space-y-2"><label className="text-[10px] uppercase font-bold text-neutral-500 tracking-wider">Reason</label><textarea required rows={4} className="w-full bg-white/5 border border-white/10 rounded-xl p-4 outline-none focus:border-indigo-500 transition-colors resize-none" placeholder="Details for the administration..." /></div>
        ...
        <div className="bg-[#0A0A0A] border border-white/5 rounded-[3rem] p-10 h-full">
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-[10px] font-mono text-neutral-500 uppercase tracking-widest mt-1">Jan 05, 2026</p>
                </div>
            ...
            <div className="space-y-4">
                {TEACHER_SCHEDULE.map((item) => (
                    <div
                        key={item.id}
                        onClick={() => addToast(`Opening Class ${item.class} Details`, "info")}
                        className={`group flex items-center gap-4 p-4 rounded-2xl border transition-all cursor-pointer hover:scale-[1.02] ${item.status === 'live' ? 'bg-indigo-500/10 border-indigo-500/30' : 'bg-white/[0.02] border-white/[0.03] opacity-80 hover:opacity-100 hover:bg-white/[0.05]'}`}
                    >
                        <div className={`text-xs font-mono font-bold w-20 ${item.status === 'live' ? 'text-indigo-400' : 'text-neutral-500'}`}>
                            {item.time.split('-')[0]}
                        </div>

                        <div className="flex-1">
                            <h4 className={`text-sm font-bold ${item.status === 'live' ? 'text-white' : 'text-neutral-300'}`}>{item.subject}: {item.topic}</h4>
                            <p className="text-[10px] text-neutral-500 flex items-center gap-2 mt-1">
                                <span className="px-2 py-0.5 rounded bg-white/5 text-neutral-400">{item.class}</span>
                                <span>• {item.room}</span>
                            </p>
                        </div>

                        {item.status === 'live' && (
                            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500 text-white text-[10px] font-bold uppercase tracking-wider animate-pulse">
...
const TeacherDashboardContent = () => {
    ...
    // Stats filtered by selected class
        ...
        <div className="min-h-screen bg-[#050505] text-white pt-28 pb-24 selection:bg-indigo-500 selection:text-white overflow-x-hidden">
            ...
            <main className="px-6 max-w-6xl mx-auto space-y-10 relative z-10">
                ...
                <section className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 px-2">
                    <div className="space-y-4">
                        <h1 className="text-5xl font-black tracking-tighter text-white">Command Center<span className="text-indigo-500">.</span></h1>
                ...
                {/* ACTIONS GRID (CLEANED - 4 ITEMS) */}
                <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                    <button onClick={() => router.push(pathname + '?view=attendance')} className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/[0.05] text-left hover:bg-white/[0.04] hover:border-indigo-500/30 transition-all group relative overflow-hidden backdrop-blur-sm">
                        <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity"><Users size={80} /></div>
                        <div className="relative z-10"><div className="p-4 bg-indigo-500/10 w-fit rounded-2xl mb-6 text-indigo-400"><CheckCircle2 size={28} /></div><h2 className="text-2xl font-bold mb-2">Attendance</h2><p className="text-sm text-neutral-500">Mark daily attendance for Class {selectedClass}.</p></div>
                    </button>
                    <button onClick={() => router.push(pathname + '?view=gradebook')} className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/[0.05] text-left hover:bg-white/[0.04] hover:border-emerald-500/30 transition-all group relative overflow-hidden backdrop-blur-sm">
                        <div className="absolute bottom-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity"><FileText size={80} /></div>
                        <div className="relative z-10"><div className="p-4 bg-emerald-500/10 w-fit rounded-2xl mb-6 text-emerald-500"><PenTool size={28} /></div><h2 className="text-2xl font-bold mb-2">Gradebook</h2><p className="text-sm text-neutral-500">Update scores for Class {selectedClass}.</p></div>
                    </button>
                    <button onClick={() => router.push(pathname + '?view=broadcast')} className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/[0.05] text-left hover:bg-white/[0.04] hover:border-rose-500/30 transition-all group relative overflow-hidden backdrop-blur-sm">
                        <div className="absolute bottom-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity"><Megaphone size={80} /></div>
                        <div className="relative z-10"><div className="p-4 bg-rose-500/10 w-fit rounded-2xl mb-6 text-rose-500"><Megaphone size={28} /></div><h2 className="text-2xl font-bold mb-2">Broadcast</h2><p className="text-sm text-neutral-500">Announce to Class {selectedClass}.</p></div>
                    </button>
                    <button onClick={() => router.push(pathname + '?view=leaves')} className="p-8 rounded-[2.5rem] bg-gradient-to-br from-white/[0.02] to-transparent border border-white/[0.05] text-left hover:border-orange-500/30 transition-all group relative overflow-hidden">
                        <div className="flex justify-between items-start mb-6"><div className="p-4 bg-orange-500/10 w-fit rounded-2xl text-orange-500"><Calendar size={28} /></div>{PENDING_LEAVES.length > 0 && <span className="bg-orange-500 text-black text-xs font-bold px-3 py-1 rounded-full">{PENDING_LEAVES.length} New</span>}</div>
                        <h2 className="text-2xl font-bold mb-2">Student Leaves</h2><p className="text-sm text-neutral-500">Review pending applications.</p>
                    </button>
                </section>
            ...
            <AnimatePresence>
                ...
                {currentView === 'settings' && <SettingsModal onClose={() => router.back()} />}
            </AnimatePresence>
        </div>
    );
};
